...
def main() -> None:
    ...
    if scores["GameID"].duplicated().any():
        dupids = scores.loc[scores["GameID"].duplicated(), "GameID"].unique()
        print("WARNING: duplicate GameIDs in scores file:", list(dupids[:20]))
    else:
    ...
    for col in ["HomeTeam", "AwayTeam", "HomeScore", "AwayScore"]:
        ...
        if scol in merged.columns:
            ...
            merged.drop(columns=[scol], inplace=True)
    ...
    # Report true mismatches after canonicalization (useful, smaller, real)
    ...
    print(f"✓ Wrote merged truth file to {TRUTH_OUT}")
