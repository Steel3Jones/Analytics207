...
OUT_EVAL_PATH = DATA_DIR / "model_eval_walkforward_v32.parquet"
...
class ScoreState:
    ...
    def predict_scores(self, season: str, gender: str, home: str, away: str) -> Tuple[float, float, float]:
        ...
        expected_total = self.cfg.total_blend * ((tot_h + tot_a) / 2.0) + (1.0 - self.cfg.total_blend) * league_total
        expected_total = float(_clip(expected_total, self.cfg.clamp_total_min, self.cfg.clamp_total_max))

        if team_total > 1e-6:
            scale = expected_total / team_total
        else:
            scale = 1.0

        mu_home = float(_clip(base_home * scale, self.cfg.clamp_team_min, self.cfg.clamp_team_max))
        mu_away = float(_clip(base_away * scale, self.cfg.clamp_team_min, self.cfg.clamp_team_max))
...
def main() -> None:
    ...
    for _, r in df.iterrows():
        ...
        mu_h, mu_a, total_pred = score.predict_scores(season, gender, home, away)
        ...
        pred_total = float(_clip(total_pred, 70.0, 160.0))
        pred_home = float((pred_total + margin_final) / 2.0)
        pred_away = float(pred_total - pred_home)

        pred_home = float(_clip(pred_home, 24.0, 98.0))
        pred_away = float(_clip(pred_away, 24.0, 98.0))
