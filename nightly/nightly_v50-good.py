...
def build_games_core(truth: pd.DataFrame) -> pd.DataFrame:
    ...
    _NAME_MAP = {"Mount Blue High School": "Mount Blue"}
...
def build_teams_analytics(truth: pd.DataFrame, games: pd.DataFrame) -> pd.DataFrame:
    ...
    def build(sub: pd.DataFrame) -> pd.Series:
        ...
        valid_opp_pi = opp_pi[~np.isnan(opp_pi)]  # still needed for exp_wins below
        ...
        for _ok in opp_keys:
            ...
            if not _opp_games.empty:
                ...
                opp_wps.append(_ow / _og)
        owp    = float(np.mean(opp_wps)) if opp_wps else np.nan
        oowp   = owp * 0.9 if not np.isnan(owp) else np.nan
        ...
        # Expected wins (using OppPI/100 as opponent win prob)
        exp_wins = float(np.nansum(1.0 - (opp_pi / 100.0))) if len(valid_opp_pi) else np.nan
...
def main() -> None:
    ...
    teams = build_teams_core(games)
    atomic_write_parquet(teams, TEAMS_CORE_V50)
    ...
    build_team_of_week(games, teams, public)
    ...
    print(f"  teams_core        {TEAMS_CORE_V50}  shape={teams.shape}")
