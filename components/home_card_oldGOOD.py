...
.tr-card.ratings .tr-ribbon{ background: linear-gradient(135deg, #facc15, #eab308); }
.tr-card.tourn .tr-ribbon{ background: linear-gradient(135deg, #fb7185, #f97316); }
...
/* Page link styling */
.tr-link-wrap{ margin-top: 8px; }
.tr-link-wrap [data-testid="stPageLink"] a{
  display: block;
  width: 100%;
  text-align: center;
  padding: 8px 10px;
...
.tr-link-wrap [data-testid="stPageLink"] a:hover{
...
<div class="tr-card {v}">
  ...
  <div class="tr-sub">{sub_html}</div>
        ...
        if path:
            st.markdown('<div class="tr-link-wrap">', unsafe_allow_html=True)
